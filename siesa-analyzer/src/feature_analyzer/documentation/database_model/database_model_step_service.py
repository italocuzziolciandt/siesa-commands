    ...
    def execute(self, data_wrapper: DataWrapperModel) -> DataWrapperModel:
        try:
            ...
            if feature_toggle_instance.is_database_model_generation_enabled():
                ...
                consolidated_diagram = self._consolidate_diagram_representations(
                    procedure_content_mapping
                )
                self.logger.info("Consolidation finished.")

                self.logger.info(
                    "Applying the new name convention to the consolidated diagram..."
                )
                database_model_full_content = self._update_table_names_with_new_convention(
                    consolidated_diagram=consolidated_diagram,
                    new_name_convention_mapping=data_wrapper.tables_new_name_convention,
                )
                self.logger.info("New name convention applied.")

                data_wrapper.output_database_model_full_content = (
                    database_model_full_content
                )
            else:
                self.logger.info(
            ...
            self.logger.error(
                f"❌ Error on generating database model diagram: {error}."
            )

        return data_wrapper

    def _update_table_names_with_new_convention(
        self, consolidated_diagram: str, new_name_convention_mapping: dict[str, str]
    ) -> str:
        updated_diagram = consolidated_diagram
        for old_name, new_name in new_name_convention_mapping.items():
            # Create a regex pattern to find the old name as a whole word.
            # \b ensures we match "w0600_docto_nomina" but not "pre_w0600_docto_nomina_post".
            # re.escape() is a safeguard in case table names contain special regex characters.
            pattern = r"\b" + re.escape(old_name) + r"\b"
            updated_diagram = re.sub(pattern, new_name, updated_diagram)
